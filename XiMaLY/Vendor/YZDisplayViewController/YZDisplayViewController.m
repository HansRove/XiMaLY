...
/** 添加或减少视图  */
@property (nonatomic, strong) UIButton *editTitleView;
/** 所以标题数组 */
...
-(UIButton *)editTitleView{
    
    if (_editTitleView == nil) {
        
        _editTitleView = [[UIButton alloc] init];
        
        _editTitleView.backgroundColor = [UIColor whiteColor];
        
        [_editTitleView setImage:[UIImage imageNamed:@"iconfont-arrows"] forState:UIControlStateNormal];
        
        [_editTitleView addTarget:self action:@selector(editTitles:) forControlEvents:UIControlEventTouchUpInside];
        //添加到标题滚动视图下面
        [self.contentView insertSubview:_editTitleView aboveSubview:self.titleScrollView];
    }
...
- (void)viewDidLayoutSubviews
{
    ...
    // 设置添加删除标题按钮的frame
    self.editTitleView.frame = CGRectMake(self.view.frame.size.width - 30,0 , 30, self.titleScrollView.frame.size.height);
...
- (void)titleClick:(UITapGestureRecognizer *)tap
{
    ...
    [self clickLabelWithLabel:label];
}
...
- (void)setCurrentIndex:(NSInteger)currentIndex{
    ...
    UILabel *label = self.titleLabels[currentIndex];
    
    [self clickLabelWithLabel:label];
}

-(void)clickLabelWithLabel:(UILabel *)label{
...
#pragma mark - 添加删除标题按钮点击
-(void)editTitles:(UIButton *)btn{
    
    [[NSNotificationCenter defaultCenter] postNotificationName:YZDisplayViewEditingTitleBtnClcikNote object:nil];
}
